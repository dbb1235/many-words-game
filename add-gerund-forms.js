// Generates -ing gerund/present-participle forms from every verb
// (RUN -> RUNNING, BAKE -> BAKING, CARRY -> CARRYING — unlike -ed/-er/
// -ly, a final y is never changed before -ing), checks each candidate
// against ENABLE, adds whatever's missing. Same methodology as the
// other add-*-forms.js scripts.
...
function gerundForms(lower) {
  const forms = new Set();
  if (lower.endsWith('e')) {
    forms.add(lower.slice(0, -1) + 'ing'); // bake -> baking
    forms.add(lower + 'ing'); // see -> seeing, dye -> dyeing — kept-e is also real sometimes
  } else if (doublesFinalConsonant(lower)) {
    forms.add(lower + lower[lower.length - 1] + 'ing'); // run -> running
  } else {
    forms.add(lower + 'ing'); // teach -> teaching, carry -> carrying (y unchanged before -ing)
  }
  return [...forms];
}
...
for (const verb of VERB_WORDS) {
  for (const form of gerundForms(verb.toLowerCase())) {
...
fs.writeFileSync(path.join(__dirname, 'gerund-forms-to-add.json'), JSON.stringify(toAdd, null, 2));
